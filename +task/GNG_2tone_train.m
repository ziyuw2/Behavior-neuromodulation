function [Info] = GNG_2tone_train(app, Info, Stim, ARDU, DAQ)    
    ...
    session.repetitionReducedTrial = [0]; % the trial number when the repetition is reduced
    session.blockSize = session.tone_num * session.repetition * 2; % for accuracy calculation, 20 trials
    ...
    session.accuracyLevel = 0.8;
    session.dprimeLevel = 1.5;
    ...
    app.smallPlot.Title.String = [Info.session.date, '  ',Info.session.animalID,' Trial Type  Repetition: ', num2str(session.repetition)];
    ...
    if session.repetition > 0
        if session.rule{trial_i} == session.initRule %&& session.initRule ~= 'C'
            if trial_i - session.repetitionReducedTrial(end) >= session.blockSize && session.accuracy(trial_i) >= session.accuracyLevel && session.dprime(trial_i) >= session.dprimeLevel
                session.repetition = session.repetition - 1;
                session.repetitionReducedTrial(end+1) = trial_i;
                app.repSwitch.Text = num2str(session.repetitionReducedTrial(end));
                [Init.trialTypeDir, Init.tone] = taskHelper.CC_updateCStypeDir(Init.trialTypeDir, trial_i, session.repetition, session.tone_num, session.initRule);
                hold(app.smallPlot, 'off'); % trial plot  
