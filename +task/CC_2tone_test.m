function [Info] = CC_2tone_test(app, Info, Stim, ARDU, DAQ)
    Info.session.date = char(datetime('now','Format','yyyy-MM-dd'));
    ...
    session.tone = {};
    session.trialType = {};
    session.repetitionReducedTrial = [0]; % only used for plotting in test session
    session.pValue = [];
    
    %% Invariable parameters
    session.windowSize = 40; % used to calculate the average lick rate, and repetition reduction
    ...
    session.USprobability = Info.taskparam.USprobability; % always deliver US for training
    ...
    while trial_i <= min(Init.nTrials, session.prespecifiedTrialNum)
        ...
        pause(session.usDelay - 0.22); % there's a short delay as processing time
        ...
        pause(session.respDur - 0.15); % there's a short delay as processing time
